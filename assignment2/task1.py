...
if case_number == 1:
    divide_file = file.map(lambda s: s.split(",")).map(lambda s: [s[0], s[1]])
else:
    divide_file = file.map(lambda s: s.split(",")).map(lambda s: [s[1], s[0]])
unique_id_file = divide_file.groupByKey().map(lambda s: [s[0],list(set(s[1]))])
